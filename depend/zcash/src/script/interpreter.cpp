...
PrecomputedTransactionData::PrecomputedTransactionData(
    ...
    const std::vector<CTxOut>& allPrevOutputs) : preTx(nullptr, zcash_transaction_precomputed_free)
{
...
PrecomputedTransactionData::PrecomputedTransactionData(
    ...
    size_t allPrevOutputsLen) : preTx(nullptr, zcash_transaction_precomputed_free)
{
...
bool PrehashedTransactionSignatureChecker::VerifySignature(
    const std::vector<unsigned char>& vchSig,
    const CPubKey& pubkey,
    const uint256& sighash) const
{
...
bool PrehashedTransactionSignatureChecker::CheckSig(
    const vector<unsigned char>& vchSigIn,
    const vector<unsigned char>& vchPubKey,
    const CScript& scriptCode,
    uint32_t consensusBranchId) const
{
    ...
    std::copy_n(this->sighash, sighashArray.size(), sighashArray.begin());
    uint256 sighash = uint256::FromRawBytes(sighashArray);
    if (!VerifySignature(vchSig, pubkey, sighash))
        return false;
...
bool PrehashedTransactionSignatureChecker::CheckLockTime(const CScriptNum& nLockTime) const
{
    // There are two times of nLockTime: lock-by-blockheight
    // and lock-by-blocktime, distinguished by whether
    // nLockTime < LOCKTIME_THRESHOLD.
